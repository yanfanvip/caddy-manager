...
## 简介
本文件面向 Caddy Panel 项目，系统性梳理其九大核心功能特性：网站管理、服务规则管理、HTTPS 证书管理、动态证书选择、OAuth 访问控制、用户管理、Token 鉴权、SSH/终端管理、运行监控、防火墙安全防护。每个功能均给出简要说明、典型使用场景与对应的实现要点，帮助读者快速理解与上手。
...
## 结论
Caddy Panel 项目围绕"统一管理"目标，提供了从监听器启停、服务规则配置、证书管理、OAuth 访问控制、用户与 Token 鉴权、SSH/终端管理到运行监控的完整能力闭环。**更新** 新增的防火墙安全防护功能进一步完善了安全体系，提供基于IP和国家的访问控制能力。
